...
`iter_token_batches` streams encoded records in stable order. A separator token
ID is mandatory because the tokenizer default does not append an end token.
Packing retains the transition between adjacent windows and can either drop an
incomplete final batch or return its complete rows.

The iterator is intentionally deterministic and does not own shuffling. A caller
can resume the same stream by skipping `Trainer.microbatches_seen` batches with
`itertools.islice`. Dataset shuffling and distributed sampling will be added
together when multi-process training has an exercised launcher.
